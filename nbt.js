...
const varintJson = JSON.stringify(require('./nbt-varint.json')).replace(
  /([if][0-7]+)/g,
  'l$1'
);
...
const hasGzipHeader = (data) => data[0] === 0x1f && data[1] === 0x8b;
...
async function parseAs(data, type, options = {}) {
  if (hasGzipHeader(data)) {
    data = Buffer.from(pako.ungzip(data));
  }
...
async function parse(data, format, callback) {
  ...
  const verifyEOF = ({ buffer, size }) => {
    ...
    if (readLen < bufferLen && !nextNbtTag) {
      throw new Error(
        `Unexpected EOF at ${readLen}: still have ${
          bufferLen - readLen
        } bytes to read !`
      );
    }
...
function equal(nbt1, nbt2) {
  ...
  if (nbt1.type === 'longArray') {
    ...
    for (let i = 0; i < nbt1.value.length; i++) {
      if (
        nbt1.value[i][0] !== nbt2.value[i][0] ||
        nbt1.value[i][1] !== nbt2.value[i][1]
      )
        return false;
    }
